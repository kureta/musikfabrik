...
def multiline_time_plot(values, name):
    ...
    return wandb.plot.line_series(  # pyright: ignore[reportPrivateImportUsage]
        xs=time,
        ys=values,
...
class DDSP(LightningModule):
    ...
    def validation_step(self, batch, batch_nb):
        ...
        if batch_nb == 0:
            ...
            for wav, m_amp, overtones, noises in zip(
                y, master_amp, overtone_amps, noise_ctrl
            ):
    ...
    def configure_optimizers(self):
        if (
            self.hparams.optim  # pyright: ignore[reportAttributeAccessIssue]
            is not None
        ):
            optim = self.hparams.optim(  # pyright: ignore[reportAttributeAccessIssue]
                self.parameters()
            )

            if (
                self.hparams.lr_scheduler  # pyright: ignore[reportAttributeAccessIssue]
                is not None
            ):
                sched = [
                    self.hparams.lr_scheduler(  # pyright: ignore[reportAttributeAccessIssue]
